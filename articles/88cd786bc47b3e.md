...
このエントリーはエンジニア転職の[LAPRAS](https://lapras.com/)/エンジニア採用の[LAPRAS SCOUT](https://scout.lapras.com/)を提供する、[LAPRAS のアドベントカレンダー](https://qiita.com/advent-calendar/2025/lapras)の 15 日目のエントリーです。
...
そう考えて、AWS のコンソール画面から ASG(Auto Scaling Group)のインスタンスの`希望するキャパシティ`を増やしたのですが……
...
![どうしてこうなったアスキーアート](https://storage.googleapis.com/zenn-user-upload/dbee91c4e1e0-20251215.png)
...
復旧作業後に分かったことなのですが、どうやらマネージド型ノードグループを削除する際に、**削除対象のノードグループで使われていた IAM ロールが他のマネージドノードグループで使用されていなければ、ロールとユーザーグループとのマッピングが aws-auth ConfigMap から削除されてしまう**と言うの EKS の仕様なようです。
...
_AWS 公式でしっかり注意書きされていた…… by [クラスターからマネージドノードグループを削除する - Amazon EKS](https://docs.aws.amazon.com/ja_jp/eks/latest/userguide/delete-managed-node-group.html)_
...
実際に検証用のクラスター上のノードのハートビート頻度を見るために`kubectl get {ノード名} -o jsonpath='{.status.conditions[?(@.type=="Ready")].lastHeartbeatTime}';`を確認してみると…
...
つまり**マネージドノード消去に伴い、aws-auth CofigMap のマッピングが消えた時点で、kubelet は(API サーバーに情報を送れず)ハートビートが途絶えてしまった**ということです。
...
そしてその 50 秒ほど後に Node の STATUS は`NotReady`となります。
...
_ASG のターゲットでノードが Draining となっている_
